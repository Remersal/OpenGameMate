...
- Automatic capture begins only after ChatGPT web audio has stopped and the safe page state has remained continuously stable for the configured 10, 15, 30, or 60 seconds (default 10). One continuous idle window triggers once and rearms after activity resumes.
- Manual capture also has a configurable global hotkey. The gesture requires a modifier plus A-Z, 0-9, or F1-F12; registration conflicts preserve the prior gesture.
...
`AutomaticSendLoop` polls only bounded page/audio readiness and opens one `AutomaticPendingSend` when a new safe idle window begins. The capacity-one pending snapshots the selected 10, 15, 30, or 60-second delay at creation and requires that full duration of unchanged safe page state and WebView2 audio silence before capture or composer mutation. A later setting change cannot shorten or extend that occurrence. A continuous idle window remains latched after one attempt and is rearmed only after page or audio activity resumes. The pending operation expires after 90 seconds, never queues a second image, and never retries an ordinary failure immediately. Pausing resets the observed idle window; resuming requires a fresh configured interval.
...
The App project is the composition root. Startup displays one application shell and restores settings, never capture or runtime state. User actions create a WebView2 instance inside the shell's right-hand browser host, confirm Voice, optionally send role text, acknowledge full-display privacy risk, and start the scheduler. Closing the embedded page disposes that WebView2 instance and returns to the in-shell placeholder; reopening creates a fresh instance with the same isolated user-data directory. WebView2 work is dispatched on the WPF thread; fixed timing remains in Core.

Submission composition is capture → prepare image/text → verify attachment/text → invoke one send control → verify composer/attachment state → delete the local PNG. Automatic capture begins only after the selected 10, 15, 30, or 60 seconds of unchanged safe page state and WebView2 audio silence; nothing is captured or inserted before that gate passes. The safe state requires the expected official origin, exactly one composer, no stop control, an unambiguous send-control state in the target form, and no exact document-level Voice activity control. A Voice or page-state change during preparation abandons that occurrence as an ordinary failure without clicking send; it is not an adapter mismatch. `QuotaReached` cancels automatic work and enters `VoiceOnly`; `AdapterInvalid` cancels work and enters `AdapterError`; ordinary failures return to `Running` without an immediate retry. No branch reads a model reply.

The tray mirrors show/hide browser, send-now, pause/resume, stop, and exit. A Win32 `RegisterHotKey` binding provides user-configurable manual capture while a game is foreground; it does not synthesize input, and a conflicting replacement leaves the old binding active. Closing the browser during a run consumes at most one background recovery and requires explicit Voice confirmation before resuming. Diagnostics export belongs to the Diagnostics module and packages only allowlisted JSONL filenames after a user-selected destination.
