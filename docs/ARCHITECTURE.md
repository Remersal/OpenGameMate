...
- Automatic capture begins only after ChatGPT web audio has stopped and the safe page state has remained continuously stable for 10 seconds. One continuous idle window triggers once and rearms after activity resumes.
...
`AutomaticSendLoop` polls only bounded page/audio readiness and opens one `AutomaticPendingSend` when a new safe idle window begins. The capacity-one pending slot requires 10 continuous seconds of unchanged safe page state and WebView2 audio silence before capture or composer mutation. A continuous idle window remains latched after one attempt and is rearmed only after page or audio activity resumes. The pending operation expires after 90 seconds, never queues a second image, and never retries an ordinary failure immediately. Pausing resets the observed idle window; resuming requires a fresh 10 seconds.
