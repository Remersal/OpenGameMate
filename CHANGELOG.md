...
- Added conversation-idle automatic capture: ChatGPT web audio and the safe page state must remain stable for the selected 10, 15, 30, or 60 seconds before capture or composer work begins; the default is 10 seconds, and one continuous idle window triggers once and rearms only after activity resumes.
...
- Removed local machine paths from tracked documentation, mapped compiler paths to `/_/`, and made release builds fail when local Windows user paths are detected.
