...
- 当前文档共有 5 处明确的用户目录或仓库绝对路径；旧历史至少由提交 `3ba2353`、`c8a1549`、`c842239` 引入相关路径。
...
- 第一次重写暴露了 PowerShell 5 无 BOM 脚本的代码页风险：含中文的正则没有生效；历史改写辅助规则因此改为全 ASCII 通配表达式，并以逐提交扫描作为唯一完成判据。
- `refs/codex/turn-diffs/.../base` 是一个非提交 tree 引用，仍保留重写前的 5 处路径；为保证整个本地对象图不会因隐藏引用继续保留隐私 blob，已删除该一条明确引用。
- 全部 21 个旧提交与新提交按顺序和提交主题核对后建立映射，当前文档中的 21 处历史哈希引用已同步更新。
- 重写后最终发布物不包含任何 `.pdb`；便携目录与安装器均由发布边界隐私检查确认不含本机 Windows 用户路径。
- 最终安装器仍未做 Authenticode 签名；隐私清理不改变既有签名状态，公开测试时仍可能出现 Windows 信誉提示。
- 删除旧引用不足以从磁盘移除隐私对象；本轮在引用清理后显式过期 reflog 并执行即时 GC，随后以旧对象解析失败和 `git fsck` 无不可达对象作为闭环证据。
...
- 用户已真实验证提交 `7f3520d` 的 10 秒空闲截图链路和可配置主动截图快捷键通过。
...
- 当前产品实现与 P0 Voice 调度修复均已提交；最新验收记录提交为 `abd441f`，工作区恢复为干净状态。
...
- Phase 0 已以提交 `3ba2353` 冻结，后续真实网页能力结论继续保留为回归基线。
